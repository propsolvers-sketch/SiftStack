...
### Phase 6 — Hoover code enforcement: council resolutions (SeeClickFix retired 2026-09-05)

**Current source:** [src/hoover_council_resolutions_api.py](src/hoover_council_resolutions_api.py) — scrapes the City Council's public "Recently Proposed and/or Approved Resolutions" HTML table (`hooveralabama.gov/1480/...`; Number / Description / Approved). Nuisance titles embed the street ("…For The Property Located At 2177 Kelly Lane"). Classified into `notice_subtype="nuisance_abatement"` (weed/vegetation/debris orders → fires `early_distress`) or `"unsafe_building"` (structure declared a public nuisance / demolition → fires `demolish`). Titles carry street only, so ZIP is recovered with one Smarty lookup anchored "Hoover AL" (fallbacks Birmingham / Vestavia Hills / AL) before the tier gate; the resolution number goes to `case_number` so `seen_code_violations.json` dedup is exact. Owner enrichment normalizes spelled-out suffixes to USPS forms (`Road→RD`, `Estate→EST`) before the Jefferson E-Ring situs search and **refuses any match whose house number differs** (E-Ring prefix-matches: "405 CAHABA RIVER EST" returns 2405). Volume: ~2-6 nuisance resolutions per council meeting (2×/month) — the already-escalated subset. Live 2026-09-05: 4/4 in-tier (35244, 35226 ×2, 35216).

**Why SeeClickFix was retired:** Hoover set `private_visibility: true` on every request category ~2026-05-17. The public `/api/v2/issues` still returns pre-flip history (last Code Enforcement issue 2026-05-17) and nothing after — submit-only. Not a migration; don't re-research or try other zoom/params. The old adapter stays on disk for history but is no longer called; the Sunday health check flags it if it is.

**Legacy adapter (historical):** [src/hoover_code_enforcement_api.py](src/hoover_code_enforcement_api.py) — Hoover (Jefferson County, Tier 1 ZIP 35022 + Tier 2 35244 + adjacent 35226 / 35216) routed citizen-reported code violations through SeeClickFix's 311-style platform. The web portal is at `https://seeclickfix.com/web_portal/cfK8xFcB5G2XrMX9VzD1cLSc` and the public API is at `https://seeclickfix.com/api/v2/issues` (no auth, browser-like User-Agent + Referer required to bypass anti-bot).
