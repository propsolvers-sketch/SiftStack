...
def _find_property_uuid(notice: NoticeData) -> str | None:
    """Query DataSift by property address to find the matched record UUID.

    Uses street + zip5 as the discriminator. Returns None if no match.
    """
    ...
    if not (street and zip5):
        return None
    try:
        # DataSift's property list supports address-based search
        resp = ds._get("/property/", {"search": street, "limit": 25})
        for candidate in (resp.get("data") or []):
            cand_addr = candidate.get("address") or {}
            cand_street = (cand_addr.get("street") or "").strip()
            cand_zip = (cand_addr.get("zip5") or "").strip()[:5]
            # Match on street + zip
            if cand_street.lower() == street.lower() and cand_zip == zip5:
